...
angular.module('app.selfwork').controller('Job001Ctrl', function ($scope, $stateParams, $state, AuthApi, Session, toaster, $uibModal, $templateCache, RestfulApi, ToolboxApi, uiGridConstants, $filter, $q) {
    ...
    angular.extend(this, {
        ...
        OverSix : function(){
            if(!angular.isUndefined($vm.vmData.OL_CR_DATETIME)){
                var _year = $filter('date')($vm.vmData.OL_CR_DATETIME, 'yyyy'),
                    _queryname = null,
                    _type = null;

                if($vm.vmData.OL_CR_DATETIME < _year+'-06-30T23:59:59.999Z'){
                    console.log('上半年');
                    _type = '上半年';
                    _queryname = 'SelectOverSixFirst';
                }
                if(_year+'-07-01T00:00:00.000Z' < $vm.vmData.OL_CR_DATETIME){
                    console.log('下半年');
                    _type = '下半年';
